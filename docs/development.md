...
The first real native frame run reproduced a screenshot permission bug: root `Page.getFrameTree` omitted already-attached OOPIFs. The last-mile screenshot gate initializes the graph independently of the public frame tool, identifies each root-child branch containing a foreign/opaque descendant, acquires its root-page border quad, captures once, and rechecks authority and structural revision. Visible unauthorized branches are conservatively box-redacted inside the MV3 Service Worker before bytes cross Native Messaging; off-viewport branches require no pixel mutation. Incomplete topology/geometry still fails closed, and transient frame attach/detach during capture invalidates the image even when the final tree looks unchanged. This is pixel redaction, not child-origin content approval.
...
- Most actions remain root-document-only. Explicit same-origin ancestor-chain child reading and same-process child click/text verification are supported; no child fill/keys/scroll/batches, cross-origin frame reading, or open/upload/download tools yet. Screenshots may include redacted placeholders for foreign frame branches but never expose their pixels. Batches use only explicit current refs; new-ref bindings and richer workflow/risk boundaries remain pending.
