...
Still required by the full v2 plan: cross-origin frame authorization and reads, child paging, richer child input beyond the explicit same-process click below, OOPIF geometry/hit testing, broader navigation/retargeting races and real-page/model acceptance. `oopif:false` therefore remains correct. This is a prerequisite, not completed iframe support or Codex parity.
...
This is an increment toward full frame interaction: `frame` now combines with exact `query` as described below, and now supports known child `rootRef` scopes below; child page windows remain pending. It is now supported separately for explicit child clicks below. Child node refs and epochs are not valid root action targets. The same-origin source routing works with same-process or flat sessions in unit tests; real Chrome/Edge evidence covers same-process reading, recursive OOPIF denial, and OOPIF-to-same-process retargeting followed by successful reading. It does not prove allowed cross-origin OOPIF content or input. `sameOriginFrameRead:true` is separate from still-false `oopif` action support.
