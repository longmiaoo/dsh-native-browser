...
| Cross-origin frame data/input leak | Bounded frame metadata, same-origin ancestor checks, denied cross-origin input, and extension-local pixel redaction of root iframe branches containing foreign/opaque descendants. Incomplete topology or geometry fails closed. | Redaction hides the entire affected root branch and may remove authorized pixels nested beside foreign content; browser/renderer bugs and unsupported frame topologies are outside the guarantee. |
